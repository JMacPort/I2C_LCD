...
void I2C_ScanAddress(void) {
    uint8_t address;

    // Try all possible addresses (0 - 127)
    for(address = 0; address < 128; address++) {
        ...
        // Wait a bit for ADDR or AF (Address Fail) flag
        for(volatile int i = 0; i < 1000; i++);

        // Check if device responded (ADDR flag set)
        if(I2C1->SR1 & (1 << 1)) {
            // Device found! Clear flags
            uint8_t temp = I2C1->SR1;
            temp = I2C1->SR2;
            printf("\rDevice found at address: 0x%02X\n", address);
        }
...
int main(void) {
    ...
    I2C_ScanAddress();  // Will print results via UART
